...
class DailyOverAllExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents, WithCustomStartCell
{
    use Exportable;
    public $val;
    ...
    public function collection()
    {
        ...
        if (strtolower(loginUserRole(auth()->user()->id)) != 'employee') {
            $dailyReport = DailyReport::select('id', 'user_id', 'project_id', 'task_description', 'worked_hours', 'date');
        } else {
            $dailyReport = DailyReport::select('id', 'user_id', 'project_id', 'task_description', 'worked_hours', 'date')->where('user_id', auth()->user()->id);
        }
        ...
        if ($this->val['user_id'] != '') {
            $this->val['user_id'] = explode(',', $this->val['user_id']);
            $dailyReport->whereIn('user_id', $this->val['user_id']);
        }
        if ($this->val['project_id'] != '') {
            $dailyReport->where('project_id', $this->val['project_id']);
        }
        ...
        foreach ($dailyReport as $detail) {
            ...
            $temp = [];
            $temp['s.no'] = $s++;
            $temp['project_name'] = projectName($detail->project_id);
            if (loginUserRole(auth()->user()->id) != 'Employee') {
